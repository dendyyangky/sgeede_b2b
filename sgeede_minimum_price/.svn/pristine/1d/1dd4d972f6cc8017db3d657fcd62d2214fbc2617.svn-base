openerp.sgeede_minimum_price = function(instance, local){
	var module = instance.point_of_sale;

//override pos models.js to include minimum price
//needed to override the whole model i guess ?
...
// include minimum price check before calling validate order (no override)
    module.PaymentScreenWidget.include({
        validate_order: function(options) {
            ...
            if(currentOrder.get('orderLines').models.length > 0){
                orderLines = currentOrder.get('orderLines').models;
                for (var i = 0; i < orderLines.length; i++) {
                    orderLine = orderLines[i];
                    quantity = orderLine.quantity;
                    subTotal = orderLine.get_base_price();
                    minimumPrice = orderLine.product["minimum_price"] * quantity;
                    if(subTotal < minimumPrice){
                        this.pos_widget.screen_selector.show_popup('error',{
                            'message': _t('Price is lower than the minimum product price!'),
                            'comment': _t("Please recheck ") + orderLine.product["display_name"],
                        });
                    return;                                    
                    }
                }
